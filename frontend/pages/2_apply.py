...
st.set_page_config(page_title="Apply for Loan", page_icon="LS")
apply_theme()

st.markdown(
    f"""
    <div class="hero-shell">
        {render_eyebrow("Application intake")}
        <div class="hero-title" style="font-size: clamp(2rem, 3vw, 3rem);">Loan Application</div>
        <div class="hero-copy">
            Capture applicant, employment, and financial details in one place so the backend can store and score the case cleanly.
        </div>
    </div>
    """,
    ...
    if not first_name or not last_name:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                st.info(f"Your Application ID: {data['application_id']}. Go to the Results page to see the decision.")
            else:
